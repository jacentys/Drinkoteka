...
- **Nadawanie Premium/uprawnień**: wyłącznie ręcznie, bezpośrednio w Supabase (SQL Editor albo `service_role`) — `update profiles set is_premium = true where user_id = ...` / `insert into user_permissions (...)`. Mechanizm kodów aktywacyjnych (`redeem_code`, `redemption_codes`, `code_redemptions`, `scripts/generate_codes.py`) został **usunięty** (patrz `scripts/supabase_remove_activation_codes.sql`) — klient nadal nigdy nie przyznaje sobie uprawnień sam (RLS to blokuje), po prostu nie ma już samoobsługowej ścieżki „wpisz kod".
...
Cały DDL nowych funkcji: **`scripts/supabase_new_tables.sql`** (idempotentny — każda polityka poprzedzona `drop policy if exists`, tabele `if not exists`, funkcje `create or replace`). Uruchamiać w Supabase → SQL Editor.

Zawiera: `user_notes`, `drink_feedback`, `app_feedback`, `profiles` (+ trigger auto-tworzenia), `user_permissions`, `restricted_sources` + RLS na `drinks` i tabelach zależnych, tabele `*_translations` + RLS. **Kody aktywacyjne usunięte** — patrz `scripts/supabase_remove_activation_codes.sql` (jednorazowa migracja, dropuje `redeem_code`/`redemption_codes`/`code_redemptions`).

**`scripts/supabase_favorites.sql`** (osobny plik, jak `scripts/supabase_subscriptions.sql`) — tabela `user_favorites (user_id, drink_id)`: obecność wiersza = drink ulubiony na koncie. Trzeba uruchomić ręcznie w SQL Editor (nie jest częścią `supabase_new_tables.sql`).

**`scripts/supabase_ingredient_stock.sql`** (analogicznie) — tabela `user_ingredient_stock (user_id, ingredient_id)`: obecność wiersza = składnik fizycznie posiadany („jest") na koncie. Synchronizowana tylko surowa wartość `jest`/nie — stany pochodne `zmJest`/`zmBrak` (dostępność przez zamiennik) przeliczają się lokalnie na każdym urządzeniu (`recalculujStanyZamiennikow`), bo zależą też od lokalnego ustawienia „zamiennikiDozwolone". Też trzeba uruchomić ręcznie w SQL Editor.
...
Pliki tłumaczeń (generowane, idempotentne `on conflict do update`), wszystkie w `scripts/`: `supabase_translations_ingredients_en.sql`, `supabase_translations_drinks_en.sql`, `supabase_translations_notes_en.sql`, `supabase_translations_steps_en.sql`. Poprawki literówek w bazowej treści PL: `scripts/supabase_fix_typos_pl.sql`.
...
- Weryfikacja server-side: `supabase/functions/verify-subscription/index.ts` (Deno/Supabase Edge Function) — klient wysyła JWS transakcji, funkcja buduje JWT (ES256) i woła Apple **App Store Server API**, potem `service_role` ustawia `profiles.is_premium`/`premium_expires_at`/`premium_product_id`/`premium_original_transaction_id` (kolumny z `scripts/supabase_subscriptions.sql`).
...
**TestFlight**: build `1.0 (1)` (bez StoreKit — wysłany przed wdrożeniem subskrypcji) jedyny build **faktycznie wysłany do Beta App Review** dotąd (status „Waiting for Review" na dzień wysyłki, potem **odrzucony** — patrz pułapka niżej). Build `1.0 (3)` (pierwszy ze StoreKit + bez wsparcia iPada) i `1.0 (4)` wgrane później przez Internal Testing (automatic distribution, grupa „Internal Tester") i przetestowane end-to-end (zakup Premium przez TestFlight → `verify-subscription` → `isPremium = true`, potwierdzone działające) — ale **żaden z nich nie został jeszcze submitowany do Beta App Review dla External Testing** („Testy zewnętrzne"). Trzeba to zrobić dla builda 4, tym razem dołączając subskrypcje do submission (patrz pułapka niżej — to była przyczyna odrzucenia builda 1).

**Test/reviewer konto**: `black@530.pl`, hasło w `scripts/.reviewer_credentials` (gitignored, nigdy nie commitować do repo). Premium przyznane (pierwotnie jednorazowym kodem aktywacyjnym — mechanizm od tamtej pory usunięty, `is_premium` w `profiles` zostaje przyznane niezależnie od tego jak). Hasło ustawione ręcznie przez Supabase Admin API (`PUT /auth/v1/admin/users/{id}` z `service_role` kluczem), bo link resetujący hasło (`resetPasswordForEmail` → deep link `drinkoteka://login-callback`) nie działał — najpewniej Apple Mail Privacy Protection zużywa jednorazowy PKCE token przez automatyczne odwiedzanie linków w tle, zanim użytkownik kliknie. Osobny, nienaprawiony problem: nawet gdyby link zadziałał, appka nie pokazuje dedykowanego ekranu „Wpisz nowe hasło" po recovery — obecny flow zakłada sesję po cichu i wymaga ręcznego przejścia do Szczegóły konta → Zmień hasło.
...
- `delete_user` RPC — potwierdzone: istnieje w `scripts/supabase_new_tables.sql:315-330`, `SECURITY DEFINER`, realnie kasuje `auth.users`.
...
- **Beta App Review odrzucone dla builda 1** z powodem: *„We are unable to complete the review of the app because one or more of the In-App Purchase products have not been submitted for review. Specifically, the app includes references to paid content but the associated In-App Purchase products have not been submitted for review."* — subskrypcje (`film.post.Drinkoteka.premium.monthly`/`.yearly`) istnieją w App Store Connect, ale nie zostały **dołączone do submission**. Ta sama przyczyna (inaczej sformułowana: „we cannot locate the In-App Purchases within the app") odrzuciła też pełną wersję App Store `1.0` (build 4) w połowie lipca 2026.
- **Prawdziwy mechanizm dołączania pierwszej subskrypcji w ASC (2024+ unified Submissions UI)** — sekcja „In-App Purchases and Subscriptions" na stronie wersji App Store to ślepy zaułek: pokazuje tylko baner informacyjny, bez żadnego przycisku dodawania. Właściwa ścieżka (znaleziona dopiero metodą prób i błędów):
  1. Na stronie samej subskrypcji (Monetization → Subscriptions → produkt) kliknąć **„Add for Review"** — to tworzy/rozszerza samodzielny **Draft Submission**. Otwarcie tego draftu samo w sobie utyka z błędem „Unable to Submit for Review — add an app version for the selected platform" — to normalne, nie próbować naprawiać w tym miejscu.
  2. Na stronie **wersji App Store** kliknąć **„Add for Review"** (przycisk z rozwijaną strzałką) → w dropdownie pojawi się istniejący „Draft [Platform] Submission (N)" z kroku 1 → wybrać go, żeby dołączyć wersję do tego draftu.
  3. Wersja może należeć tylko do **jednej** submisji naraz — jeśli jest już „zaklepana" przez inną (np. przez zwykłe „Resubmit to App Review" bez subskrypcji), trzeba ją najpierw zwolnić przez **„Cancel Submission"** na tamtej submisji (bezpieczne, odwracalne).
  4. Po połączeniu wersji + subskrypcji w jednym drafcie może pojawić się czerwony błąd „There are errors with one or more of your items — remove the items and add them again" mimo kompletnych danych — naprawia się usunięciem subskrypcji z draftu i ponownym dodaniem ich pojedynczo (krok 1) po tym, jak wersja jest już dołączona.
  5. **Każda edycja metadanych wersji po dołączeniu subskrypcji do draftu potrafi ponownie zepsuć krok 4** — robić wszystkie edycje wersji najpierw, dopiero na końcu dołączać subskrypcje i od razu submitować.
  - ASC bywa też po prostu niestabilne (generyczne „Sorry, something went wrong" / „An error has occurred. Try again later" mimo zielonego system-status) — warto próbować w innej przeglądarce/incognito, zanim uzna się to za błąd konfiguracji.
  - Wymagane pola per subskrypcja: nazwa, opis, cena, **Review Screenshot** (zrzut z appki pokazujący ekran zakupu — bez niego produkt nie osiąga „Ready for Review").

**Wciąż do zrobienia**: Build `1.0 (4)` wysłany do pełnego App Store Review, odrzucony ponownie (2.1.0 App Completeness / „cannot locate the In-App Purchases") mimo że subskrypcje istnieją i są kompletne — przyczyna: **Paid Apps Agreement aktywowana dopiero 10 lipca 2026, tax forms (W-8BEN-E, Certificate of Foreign Status) złożone 13 lipca 2026** (pierwsza aktywacja, nie odnowienie) — podejrzenie o opóźnienie propagacji w backendzie Apple (ten sam mechanizm co wcześniejszy, rozwiązany sam z siebie problem z 401 przy weryfikacji zakupów). **16 lipca 2026 złożona formalna apelacja** przez developer.apple.com/contact (App Review → Request support for an App Rejection or Removal → App Rejection, dla „Drinkoteka (iOS)") — czeka się na odpowiedź Apple. Zrzuty ekranu — zrobione.

Zrobione wcześniej: `IPHONEOS_DEPLOYMENT_TARGET` obniżony do 17.0 (z fallbackami dla API iOS 18: `MeshGradient`, `toolbarBackgroundVisibility`), logi `print` zamienione na `dprint` (aktywne tylko w Debug — definicja w `Funkcje_VM.swift`), usunięty duplikat DTO i martwe loadery TSV, ikona (pop-art, 1024×1024), nazwa ujednolicona na „Drinkotheque" (bez akcentu — łatwiejsze wyszukiwanie), Privacy Policy + strona wsparcia wgrane na 530.pl (źródło HTML lokalnie w `docs/www/privacy/` i `docs/www/support/`), metadane App Store (`docs/app-store-metadata.md`), deep link potwierdzenia maila (`drinkoteka://login-callback`), animowany ekran ładowania + obsługa offline przy 1. uruchomieniu, model dostępu B (Premium wymagane dla nie-IBA, widoczne-ale-zablokowane), rola admina + faza serwerowa B (patrz wyżej), ustawienie wyglądu (systemowy/jasny/ciemny) i spójne tytuły ekranów. Sign in with Apple NIE jest wymagane (tylko email/hasło).
